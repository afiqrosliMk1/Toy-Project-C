#include <stdio.h>

void printResult( int b[] , int arraySize );

int main( void )
{
    int counter[ 9 ] = { 0 };
    //float commision = 1.9;
    int basic = 200;
    int salary;
    int sale;

    printf( "Enter sale for this week: " );
    scanf( "%d" , &sale );
    while ( sale != EOF ){
        salary = basic + 1.9 * sale;
        if ( salary >= 200 && salary <= 299 ){
            ++counter[ 0 ];
        }else if ( salary >= 300 && salary <= 399 ){
            ++counter[ 1 ];
        }else if ( salary >= 400 && salary <= 499 ){
            ++counter[ 2 ];
        }else if ( salary >= 500 && salary <= 599 ){
            ++counter[ 3 ];
        }else if ( salary >= 600 && salary <= 699 ){
            ++counter[ 4 ];
        }else if ( salary >= 700 && salary <= 799 ){
            ++counter[ 5 ];
        }else if ( salary >= 800 && salary <= 899 ){
            ++counter[ 6 ];
        }else if ( salary >= 900 && salary  <= 999 ){
            ++counter[ 7 ];
        }else{//more than 1000
            ++counter[ 8 ];
        }
        printf( "Enter sale for this week: " );
        scanf( "%d" , &sale );
    }
    printResult( counter , 9 );

    return 0;
}

void printResult( int b[] , int arraySize )
{
    static int i;
    int low = 200 , high = 299;
    for ( i = 0; i < arraySize; ++i , low += 100 , high += 100 ){
        if ( i == arraySize - 1 ){
            printf( "/$%5d and above%5d\n" , low ,  b[ i ] );
        }else{
            printf( "$%5d-$%5d%8d\n" , low , high , b[ i ] );
        }
    }


}
